...
또 하나 어려웠던 점은 비트코인 가격이 실시간으로 계속 바뀌는 만큼 그 업데이트를 화면에 자연스럽게 연결하는 부분이었다.
가격, 차트, 체결 내역이 함께 움직이도록 맞추는 과정이 쉽지 않았지만 그 과정을 통해 상태 변화와 렌더링 흐름이 실제로 어떻게 이어지는지 더 깊게 이해할 수 있었다.
